...
# Here we identify a number of alternatives.
...
scientist begins with in analyzing a new dataset. See \textit{Literate
  Data Science With R} (2017) for details. The sequence is extracted
into separate files, corresponding to the logical steps along the
way. Each can be run sequentially.
...
fpath <- "data"
fname <- file.path(fpath, dsname %s+% ".RData") %T>% print() %%UPDATE TO ESSENTIALS%%XXXXXXXX
