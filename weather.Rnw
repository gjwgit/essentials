...
# Welcome to the Togaware Data Science End-to-End Template WeatherAUS ----
# 
...
\clearpage
\section{Deal with Numeric Variables}

In our dataset we might notice that evaporation and sunshine have come
through as character variables. This is because there are only missing
values in the first many observations and the heuristics interprets
that as character variables.

<<template:numeric_data>>=
